...
<div class="container" id="container">
    ...
    <div class="form-container sign-in-container">
        <form action="{{ route('signup') }}" method="post">
            ...
            <br>
            <label for="email" style="font-size:18px;"> Email : {{ request('email') }} </label>
            <input type="email" name="email" id="email" placeholder="{{ request('email') }}"
                   class="@error('email') border-danger @enderror" value="{{ request('email') }}" onchange="preventEdit()" readonly hidden>
            @error('email')
